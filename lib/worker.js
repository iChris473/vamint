 ...
 */
// GPU by default: SolVanityCL ships inside this repo, so with no env config
// at all we run `python3 <repo>/SolVanityCL/main.py`. Env vars only override.
const REPO_ENGINE_DIR = path.resolve(__dirname, '..', 'SolVanityCL')

class GrinderClient {
  constructor({
    pythonBin   = process.env.GRINDER_PYTHON || 'python3',
    engineDir   = process.env.GRINDER_ENGINE_DIR || REPO_ENGINE_DIR,
  ...
  async init() {
    if (this.cpuFallback) {
      console.log('[grinder] CPU fallback explicitly enabled (GRINDER_CPU_FALLBACK=true)')
      return
    }
    if (!fs.existsSync(this.entry)) {
      console.warn(
        `[grinder] SolVanityCL not found at ${this.entry} — CPU fallback ` +
        `(set GRINDER_ENGINE_DIR or restore the bundled repo)`,
      )
      this.cpuFallback = true
      return
    }
    // Probe the engine once so a broken python env (missing pyopencl/click,
    // wrong interpreter) degrades to CPU at boot instead of failing every job.
    const probe = spawnSync(this.pythonBin, [this.entry, '--help'], {
      cwd: this.engineDir,
      timeout: 15_000,
    ...
    if (probe.error || probe.status !== 0) {
      const detail = probe.error
        ? probe.error.message
        : (probe.stderr?.toString().trim().slice(-200) || `exit code ${probe.status}`)
      console.warn(`[grinder] engine probe failed — CPU fallback (${detail})`)
      this.cpuFallback = true
      return
    }
    fs.mkdirSync(this.workDir, { recursive: true })
    this.procReady = true
    console.log(`[grinder] OpenCL engine ready (${this.pythonBin} ${this.entry})`)
  }
