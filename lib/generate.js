...
// Grind constraint (mirrored on the frontend): EITHER a prefix OR a suffix
// (never both), max 4 characters.
const MAX_LEN = 5
...
function validateSpec({ prefix = '', suffix = '', caseSensitive = true }) {
  ...
  if (!prefix && !suffix) {
    throw makeErr(400, 'a prefix or a suffix is required')
  }
  if (prefix && suffix) {
    throw makeErr(400, 'choose either a prefix or a suffix — not both')
  }
  if (!isValidBase58(prefix) || !isValidBase58(suffix)) {
    throw makeErr(400, 'prefix/suffix must use base58 chars only (no 0, O, I, l)')
  }
  if (prefix.length > MAX_LEN || suffix.length > MAX_LEN) {
    throw makeErr(400, `max ${MAX_LEN} characters`)
  }
  return { prefix, suffix, caseSensitive: !!caseSensitive }
}

async function startGenerateJob(rawSpec, opts = {}) {
  // waiveFee is server-side only (admin route) — never derived from rawSpec,
  // so API callers can't set it.
  const waiveFee = !!opts.waiveFee
  ...
  const job = {
    ...
    payPoller: null,
    stderrTail: '',
    // fee-waived wallet jobs are born paid: keys release straight on 'done'
    paid: kind === 'wallet' && waiveFee,
    ground: false,
    grindMs: null,
    feeToken,
    depositPubkey: null,
  ...
  if (kind === 'wallet' && waiveFee) {
    console.log(`[generate] job ${id} fee waived (admin) — grinding immediately`)
  }
  if (kind === 'wallet' && !waiveFee) {
    ...
    deposits.record({
      ...
      secret: deposit.secretKey,
      minRaw: feeToken.minRaw,
      deadline: job.paymentDeadline,
    })
    console.log(
...
function markPaid(job) {
  ...
  // flag the ledger first so the recovery scanner knows this balance is
  // legitimately ours (unpaid balances get refunded, never swept)
  deposits.markPaid(job.depositPubkey)
  sweepDeposit(job)
  if (job.ground) {
    job.status = 'done'
...
function watchPayment(job) {
  ...
  job.payPoller = setInterval(async () => {
    ...
    if (Date.now() > job.paymentDeadline) {
      // wipe any held keys — nothing was paid for. A partial payment (short
      // by more than the tolerance) goes back to whoever sent it.
      finalize(job, {
        status: 'expired',
        error: 'deposit window expired — no payment received',
        secretKey: null,
      })
      refundUnderpayment(job)
      return
    }
    if (inFlight) return
    inFlight = true
    try {
      const paid = await tokenPaymentReceived(job.depositPubkey, job.feeToken.minRaw || job.feeToken.amountRaw)
...
// An expired job may hold a partial payment (short by more than the
// tolerance) — send it back to the sender. Best-effort: the recovery
// scanner retries from the durable ledger if this fails.
function refundUnderpayment(job) {
  if (!job.depositSecret) return
  refundDeposit(job.depositSecret)
    .then(r => {
      if (!r) return
      deposits.markRefunded(job.depositPubkey, r.signature, r.to)
      console.log(
        `[generate] refunded underpayment for ${job.id}: ` +
        `${r.tokensRaw} raw tokens → ${r.to} (${r.signature})`,
      )
    })
    .catch(err => console.warn(
      `[generate] refund failed for ${job.id}: ${err.message} — recovery scanner will retry`,
    ))
}
