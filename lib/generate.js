 ...
 *      POST /:jobId/confirm button. Once the deposit holds
 *      GENERATE_FEE_LAMPORTS (0.05 SOL): ground already → 'done' (keys
 *      released); still grinding → 'processing' → 'done' when found.
 *   4. If the window expires unpaid → 'expired' and any held keys are
 *      wiped. On release, the deposit is swept to OPERATOR_WALLET
 *      best-effort.
 *
...
function startGenerateJob(rawSpec) {
  ...
  if (kind === 'wallet') {
    ...
    console.log(
      ...
      `deposit at ${deposit.publicKey} (grinding starts now, keys held until paid)`,
    )
    watchPayment(job)
  }
...
// Deposit confirmed: release held keys, or keep grinding with the gate open.
