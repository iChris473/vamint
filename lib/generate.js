...
// bs58 v6 is ESM-only, so `require('bs58')` returns the module namespace —
// the encoder/decoder lives on `.default`. v5 returns the encoder directly.
// Handle both so npm-install drift never breaks parsing again.
...
async function generateVanityKeypair({
  ...
  // ─── spawn engine ────────────────────────────────────────────────────────
  console.log(
    `[generate] spawning SolVanityCL (cwd=${ENGINE_DIR}) ` +
    `prefix=${prefix || '∅'} suffix=${suffix || '∅'} caseSensitive=${caseSensitive}`,
  )
...
function harvest(outDir, prefix, suffix, caseSensitive) {
  ...
  for (const f of files) {
    ...
    // writes exactly that layout, so we can hand it straight in.
    let derivedPubBytes
    try {
      derivedPubBytes = nacl.sign.keyPair.fromSecretKey(fullKey).publicKey
    } catch (e) {
      console.warn(`[generate] tweetnacl rejected ${f}: ${e.message}`)
      continue
    }
    ...
    try { derivedPub = bs58.encode(derivedPubBytes) }
      ...
      console.warn(`[generate] bs58.encode threw — check bs58 version/interop: ${e.message}`)
      continue
    }
    ...
    if (pfx && !cmp.startsWith(pfx)) {
      console.warn(`[generate] ${address} does not start with ${pfx}`)
      continue
    }
    if (sfx && !cmp.endsWith(sfx)) {
      console.warn(`[generate] ${address} does not end with ${sfx}`)
      continue
    }
